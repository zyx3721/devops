...
func init() {
	ioc.Api.RegisterContainer("DingtalkHandler", &DingtalkApiHandler{})
}

type DingtalkApiHandler struct {
	handler *DingtalkHandler
}

func (h *DingtalkApiHandler) Init() error {
	cfg, _ := config.LoadConfig()
	h.handler = NewDingtalkHandler(cfg.GetDB())

	root := cfg.Application.GinRootRouter().Group("dingtalk")
	h.Register(root)
	return nil
}

func (h *DingtalkApiHandler) Register(r gin.IRouter) {
...
type DingtalkHandler struct {
	...
	appRepo *repository.DingtalkAppRepository
	botRepo *repository.DingtalkBotRepository
	logRepo *repository.DingtalkMessageLogRepository
	db      *gorm.DB
}

func NewDingtalkHandler(db *gorm.DB) *DingtalkHandler {
	return &DingtalkHandler{
		logger:  logger.NewLogger("INFO"),
		appRepo: repository.NewDingtalkAppRepository(db),
		botRepo: repository.NewDingtalkBotRepository(db),
		logRepo: repository.NewDingtalkMessageLogRepository(db),
		db:      db,
	}
}

// SendMessage 发送工作通知消息
func (h *DingtalkHandler) SendMessage(c *gin.Context) {
	var req struct {
	...
	client := dingtalk.NewClient(app.AppKey, app.AppSecret, app.AgentID)
	...
	case "markdown":
		msg = map[string]interface{}{
			"msgtype":  "markdown",
			"markdown": map[string]string{"title": req.Title, "text": req.Content},
		}
	default:
	...
	err = client.SendWorkMessage(c.Request.Context(), req.UserIDList, req.MsgType, msg)

	// 记录日志
	logEntry := &models.DingtalkMessageLog{
		MsgType: req.MsgType,
		Target:  req.UserIDList,
		Content: req.Content,
		Title:   req.Title,
		Source:  "manual",
		Status:  "success",
...
func (h *DingtalkHandler) SendWebhook(c *gin.Context) {
	var req struct {
	...
	client := dingtalk.NewClient("", "", 0)

	// 构建消息
	msg := &dingtalk.WebhookMessage{
		MsgType: req.MsgType,
		At: &dingtalk.WebhookAt{
			AtUserIds: req.AtUsers,
			IsAtAll:   req.AtAll,
		},
	}
	...
	switch req.MsgType {
	case "text":
		msg.Text = &dingtalk.WebhookText{Content: content["content"]}
	case "markdown":
		msg.Markdown = &dingtalk.WebhookMarkdown{Title: content["title"], Text: content["text"]}
	}

	err = client.SendWebhookMessage(c.Request.Context(), bot.WebhookURL, bot.Secret, msg)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": 500, "message": err.Error()})
		return
	}
...
// SearchUser 搜索用户
func (h *DingtalkHandler) SearchUser(c *gin.Context) {
	var req struct {
...
func (h *DingtalkHandler) ListMessageLogs(c *gin.Context) {
...
// 应用管理
func (h *DingtalkHandler) ListApps(c *gin.Context) {
	page, _ := strconv.Atoi(c.DefaultQuery("page", "1"))
...
func (h *DingtalkHandler) GetApp(c *gin.Context) {
...
func (h *DingtalkHandler) CreateApp(c *gin.Context) {
	var app models.DingtalkApp
...
func (h *DingtalkHandler) UpdateApp(c *gin.Context) {
	id, _ := strconv.ParseUint(c.Param("id"), 10, 64)
	var app models.DingtalkApp
	if err := c.ShouldBindJSON(&app); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"code": 400, "message": err.Error()})
...
func (h *DingtalkHandler) DeleteApp(c *gin.Context) {
	id, _ := strconv.ParseUint(c.Param("id"), 10, 64)
...
func (h *DingtalkHandler) SetDefaultApp(c *gin.Context) {
	id, _ := strconv.ParseUint(c.Param("id"), 10, 64)
...
// GetAppBindings 获取钉钉应用绑定的 Jenkins 实例和 K8s 集群
func (h *DingtalkHandler) GetAppBindings(c *gin.Context) {
	id, _ := strconv.ParseUint(c.Param("id"), 10, 64)
	...
	h.db.Table("jenkins_instances").
		...
		Joins("JOIN jenkins_dingtalk_apps ON jenkins_dingtalk_apps.jenkins_instance_id = jenkins_instances.id").
		Where("jenkins_dingtalk_apps.dingtalk_app_id = ?", id).
	...
	h.db.Table("k8s_clusters").
		...
		Joins("JOIN k8s_cluster_dingtalk_apps ON k8s_cluster_dingtalk_apps.k8s_cluster_id = k8s_clusters.id").
		Where("k8s_cluster_dingtalk_apps.dingtalk_app_id = ?", id).
		Scan(&k8sBindings)
...
// 机器人管理
func (h *DingtalkHandler) ListBots(c *gin.Context) {
	page, _ := strconv.Atoi(c.DefaultQuery("page", "1"))
	pageSize, _ := strconv.Atoi(c.DefaultQuery("page_size", "100"))
...
func (h *DingtalkHandler) GetBot(c *gin.Context) {
	id, _ := strconv.ParseUint(c.Param("id"), 10, 64)
...
func (h *DingtalkHandler) CreateBot(c *gin.Context) {
	var bot models.DingtalkBot
...
func (h *DingtalkHandler) UpdateBot(c *gin.Context) {
	id, _ := strconv.ParseUint(c.Param("id"), 10, 64)
	var bot models.DingtalkBot
	if err := c.ShouldBindJSON(&bot); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"code": 400, "message": err.Error()})
...
func (h *DingtalkHandler) DeleteBot(c *gin.Context) {
	id, _ := strconv.ParseUint(c.Param("id"), 10, 64)
